...
void ls7a_intc_init(void)
{
	...
	WR_PORT(LS7A_INT_MASK_REG, ~((0x1UL << UART0_IRQ) | (0x1UL << KEYBOARD_IRQ) |
								 (0x1UL << MOUSE_IRQ) | (0x1UL << DISK_IRQ)));

	WR_PORT(LS7A_INT_EDGE_REG, (0x1UL << (UART0_IRQ | KEYBOARD_IRQ | MOUSE_IRQ | DISK_IRQ)));

	/* route to the same irq in extioi */
	WR_PORT(LS7A_INT_HTMSI_VEC_REG + UART0_IRQ, UART0_IRQ);
	WR_PORT(LS7A_INT_HTMSI_VEC_REG + KEYBOARD_IRQ, KEYBOARD_IRQ);
	WR_PORT(LS7A_INT_HTMSI_VEC_REG + MOUSE_IRQ, MOUSE_IRQ);
	// WR_PORT(LS7A_INT_HTMSI_VEC_REG + DISK_IRQ, DISK_IRQ);
	...
	// *(volatile unsigned char *)(LS7A_INT_HTMSI_VEC_REG + DISK_IRQ) = MOUSE_IRQ;

	WR_PORT(LS7A_INT_POL_REG, 0x0UL);
...
{
	WR_PORT(LS7A_INT_CLEAR_REG, (irq));
}
