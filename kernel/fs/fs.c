...
void* fs_info_malloc(int size){
    ...
    if(fs_info_alloc_start>0x122000000) return 0;
    return p;
}
...
void *fs_malloc(int size){
    ...
    fs_alloc_start+=size;
    if(fs_alloc_start>0x130000000) return 0;
    return p;
}
...
void create_folder(struct file_folder *relative_path, char folder_name[]){
    ...
    if(relative_path->file_folder_list[list_n] == 0) printf("No room to create folder!\n");
...
void *path_als(char path[],int type){ // 0 folder 1 file 2 folder of file
    int depth;
    char split_path[100][100];
    if (path[0]=='/') split(&path[1],"/",split_path,&depth);
    else split(path,"/",split_path,&depth);
    struct file_folder *dst_p = root_path;
    
    if(strcmp(path, "/") == 0 && type == 0) return root_path;
    ...
    if(type == 0){
        ...
        if(type == 1) dst_p = dst_p->file_list[list_n];
    }
    return (void*)dst_p;
}
